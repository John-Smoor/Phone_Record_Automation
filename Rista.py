...
# ── Paths ─────────────────────────────────────────────────────────────────────
# Resolve all paths relative to this script's location so they work on both
# Windows (local) and Linux (GitHub Actions) without any hardcoding.
BASE_DIR    = os.path.dirname(os.path.abspath(__file__))
DATA_FILE   = os.path.join(BASE_DIR, "data", "sales_rolling.csv")
BRANCH_FILE = os.path.join(BASE_DIR, "data", "branch_codes.csv")
SEED_CSV    = os.path.join(BASE_DIR, "data", "sales_rollings.csv")
...
def update_rolling_csv(new_rows):
    ...
    # os.makedirs is safe even if the directory already exists
...
#  ENTRY POINT
#
#  python Rista.py            →  daily mode  (add yesterday, drop oldest)
#  python Rista.py backfill   →  backfill mode  (build 21 days from scratch)
# ═════════════════════════════════════════════════════════════════════════════
