...
def build_report(input_csv: str, output_xlsx: str):
    ...
    L.log(f"  Data cutoff   : {yesterday} (yesterday — today's records excluded)")
    ...
    # ── STEP 3: Parse createdDate → date & filter to yesterday ───────────────
    L.section("STEP 3 — PARSE createdDate & FILTER TO YESTERDAY")
    L.log("  Using 'createdDate' column (datetime) converting to date only.")
    L.log(f"  Keeping only records where date <= {yesterday}")

    df['date'] = pd.to_datetime(df['createdDate'], errors='coerce').dt.date

    invalid_dates      = df['date'].isna().sum()
    L.log(f"  Rows with valid createdDate         : {df['date'].notna().sum():,}")
    L.log(f"  Rows with invalid/missing createdDate (DROPPED): {invalid_dates:,}")
    df = df[df['date'].notna()].copy()

    rows_before_cutoff = len(df)
    df = df[df['date'] <= yesterday].copy()
    L.log(f"  Rows DROPPED (today or future date) : {rows_before_cutoff - len(df):,}")
    L.log(f"  Rows KEPT (up to yesterday)         : {len(df):,}")

    # ── STEP 4: Deduplicate at invoice level ──────────────────────────────────
    L.section("STEP 4 — DEDUPLICATE AT INVOICE LEVEL")
    before_dedup = len(df)
    ...
    L.log("  Phone number is taken from the first line item of each invoice.")

    # ── STEP 5: City / Format mapping ─────────────────────────────────────────
    L.section("STEP 5 — BRANCH MAPPING (City & Format)")
    ...
    inv = inv[inv['branchName'] != 'RBI Officers Canteen']
    ...
    # ── STEP 6: Week / Day buckets ────────────────────────────────────────────
    L.section("STEP 6 — WEEK & DAY BUCKETS")
    ...
    # ── Only current WTD week + last 1 day ───────────────────────────────────
    all_yws    = sorted(inv['yw'].dropna().unique())
    latest_yw  = all_yws[-1] if all_yws else None

    # Only the current (WTD) week
    target_week_pairs = [(int(latest_yw // 100), int(latest_yw % 100))] if latest_yw else []

    all_days    = sorted(inv['date'].dropna().unique())
    target_days = all_days[-1:] if all_days else []   # only yesterday (last day)

    L.log(f"  Date range in data                  : {min(all_days)} → {max(all_days)}")
    L.log(f"  Total unique days in data           : {len(all_days)}")
    L.log()
    L.log("  WTD week selected:")
    for (yr, wk) in target_week_pairs:
        yw_key = yr * 100 + wk
        wk_inv = inv[(inv['iso_week'] == wk) & (inv['iso_year'] == yr)]
        L.log(f"    Year {yr}  Week {wk:02d}  (yw={yw_key})  →  {wk_inv['invoiceNumber'].nunique():,} invoices  ← WTD")
    L.log()
    L.log("  Last day selected:")
    for d in target_days:
        day_inv = inv[inv['date'] == d]
        L.log(f"    {d}  →  {day_inv['invoiceNumber'].nunique():,} invoices across {day_inv['branchName'].nunique()} branches")

    # ── STEP 7: Branch-level metrics ──────────────────────────────────────────
    L.section("STEP 7 — BRANCH-LEVEL CAPTURE RATE CALCULATIONS")
    L.log()
    L.log("  FORMULA:")
    ...
    def capture(sub):
        ...
        has_phone = sub['clean_phone'].nunique()
    ...
    L.section("STEP 8 — OVERALL SUMMARY")
    ...
    # Columns:  1=City  2=Branch  3=Format  | 4..4+nW-1 = week cols | 4+nW..end = day cols
    n_fixed    = 3
    n_week     = len(week_cols)
    n_day      = len(day_cols)
    total_cols = n_fixed + n_week + n_day

    week_col_start = n_fixed + 1                    # first week col index
    week_col_end   = n_fixed + n_week               # last  week col index
    day_col_start  = n_fixed + n_week + 1           # first day  col index
    day_col_end    = total_cols                      # last  day  col index
    ...
    # ── Row 3: Group headers (DYNAMIC spans) ──────────────────────────────────
    # Fixed cols: blank with royal-blue fill
    for c_idx in range(1, n_fixed + 1):
        cell = ws.cell(row=3, column=c_idx)
        cell.fill = PatternFill('solid', start_color=ROYAL_BLUE)

    # Week group header — WTD label
    if n_week > 0:
        wk_cell = ws.cell(row=3, column=week_col_start)
        wk_cell.value     = "Week to Date (WTD)"
        wk_cell.font      = Font(name='Arial', bold=True, size=9, color=WHITE)
        wk_cell.fill      = PatternFill('solid', start_color=HDR_WEEK)
        wk_cell.alignment = Alignment(horizontal='center', vertical='center')
        wk_cell.border    = bdr

    # Day group header — Yesterday label
    if n_day > 0:
        dy_cell = ws.cell(row=3, column=day_col_start)
        dy_cell.value     = "Yesterday"
        dy_cell.font      = Font(name='Arial', bold=True, size=9, color=WHITE)
        dy_cell.fill      = PatternFill('solid', start_color=HDR_DAY)
        dy_cell.alignment = Alignment(horizontal='center', vertical='center')
        dy_cell.border    = bdr
    ...
    for ci, h in enumerate(headers, start=1):
        ...
        if ci <= n_fixed:
            bg, fg = ROYAL_BLUE, WHITE
        elif ci <= week_col_end:
            bg, fg = HDR_WEEK, WHITE
        else:
            bg, fg = HDR_DAY, WHITE
    ...
    ws.merge_cells(start_row=total_row_num, start_column=1,
                   end_row=total_row_num,   end_column=n_fixed)
    ...
    lbl_cell.border    = bdr
    for c_idx in range(2, n_fixed + 1):
        ws.cell(row=total_row_num, column=c_idx).border = bdr

    for ci, col in enumerate(metric_cols, start=n_fixed + 1):
        val  = totals.get(col)
    ...
    # ── Conditional formatting on day columns (red→white) ─────────────────────
    cf_last_row = total_row_num - 1
    ...
    # ── Column widths ─────────────────────────────────────────────────────────
    ws.column_dimensions['A'].width = 14   # City
    ws.column_dimensions['B'].width = 28   # Branch
    ws.column_dimensions['C'].width = 14   # Format
    for ci in range(n_fixed + 1, total_cols + 1):
        ws.column_dimensions[get_column_letter(ci)].width = 9

    ws.freeze_panes = get_column_letter(n_fixed + 1) + '5'
    ...
    raw = raw[raw['branchName'] != 'RBI Officers Canteen']
    ...
    raw_dump = (
        ...
        .drop_duplicates(subset='clean_phone', keep='first')
...
if __name__ == "__main__":
    ...
    ROLLING_CSV = r"C:/Users/jeryy/OneDrive - BLISS CHOCOLATES INDIA PRIVATE LIMITED/Documents/Phone No/Data/sales_rolling.csv"
